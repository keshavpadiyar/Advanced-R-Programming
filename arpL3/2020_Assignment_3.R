...
#' @export
#' 
#' @details This function finds greatest common divisor of the given 2 numbers. If inputs are not numeric scala,
...
#' Find the shortest path Between Verities using Dijkstra's algorithm
#'
#' @param data Data frame consisting verities and distance between them 
#' @param node Initial Node/vertex from which the distance is to be calculated 
#'
#' @return Shortest distance from the initial node/Vertex to all other verities
#' @export
#'
#' @details This function finds the shortest distance from the input vertex to all other
#'verities.
#'    
#'For additional information click on the below URL
...
#' validate if the input data is a data frame of 3 columns
...
#' Initialize the distance from the required node to itself as Zero
  vl_weight[node] <- 0
#' Run the loop till all the verities/Nodes are not traversed
